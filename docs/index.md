...
[GOCACTL.md](gocactl.md) — `gocactl`, the remote client: managing a goca
...
- **Two binaries, one release.** `goca` is the server and its local CLI.
  ...
  renders identical output to `goca`. See **[GOCACTL.md](gocactl.md)**.
...
[Dockerfile](https://github.com/mevijays/goca/blob/main/Dockerfile) for what's in it (two static binaries —
...
The same roles govern [`gocactl`](gocactl.md), since the server does the
